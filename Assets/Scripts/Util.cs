...
public class Util {
  ...
  /// ※fileNameに空文字（""）を指定するとシングルスプライトから取得します.
	...
	public static void SetFont(){
		Font font = Resources.Load<Font> ("Fonts/GenEi");
		GetGUIStyle ().font = font;
	}
  /// フォントサイズを設定.
  ...
  public static bool GUIButton(float x, float y, float w, float h, string text) {
    ...
    rect.height = h;
		...
		style.font = GetGUIStyle ().font;
		style.fontSize = GetGUIStyle ().fontSize;
		style.normal.textColor = GetGUIStyle ().normal.textColor;
		style.alignment = GetGUIStyle ().alignment;

		return GUI.Button(rect, text, style);
	}
	public static bool GUIButton(float x, float y, float w, float h, Texture2D texture) {
		Rect rect = GetGUIRect();
		rect.x = x;
		rect.y = y;
		rect.width = w;
		...
		return GUI.Button(rect, texture, style);
	}
}
